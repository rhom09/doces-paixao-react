...
export function Sobre() {
  const { data: sanityData } = useSanity<AboutData>(ABOUT_QUERY)

  const label = sanityData?.label || '✦ Nossa História'
  const title = sanityData?.title || 'Uma Paixão que se Tornou <em class="italic text-rose">Arte Doce</em>'
  const content = sanityData?.content || [
    'A Doces Paixão nasceu em 2014, de um sonho apaixonante de criar doces que encantam os olhos e aquecem o coração. Cada receita é desenvolvida com carinho, utilizando ingredientes frescos e de altíssima qualidade.',
    'Nossa chef confeiteira, Maria Clara, traz mais de uma década de experiência em confeitaria artesanal, com especialização em cake design e técnicas francesas de pâtisserie.',
  ]
  const features = sanityData?.features || DEFAULT_FEATURES
  const badgeTitle = sanityData?.badgeTitle || 'Ingredientes Selecionados'
  const badgeSubtitle = sanityData?.badgeSubtitle || 'Qualidade Premium'

  const imageUrl = sanityData?.image
    ? urlFor(sanityData.image).width(700).url()
    : 'https://images.unsplash.com/photo-1556217477-d325251ece38?w=700&q=85'
      ...
      <div className="container mx-auto grid max-w-[1180px] grid-cols-1 items-center gap-[80px] px-7 lg:grid-cols-2">
        ...
        <RevealWrapper direction="left" className="relative pb-10 pl-5 pt-5">
          ...
          <div className="absolute -right-3 bottom-0 z-20 flex items-center gap-3.5 rounded-[20px] border border-border-soft bg-white px-5 py-4 shadow-md">
            ...
            <div>
              <strong className="block font-display text-[1.3rem] leading-tight text-ink">{badgeTitle}</strong>
              <small className="text-[0.78rem] text-muted">{badgeSubtitle}</small>
            </div>
          </div>
        </RevealWrapper>
        ...
        <RevealWrapper direction="right" className="pl-2.5">
          <LabelTag className="mb-4">{label}</LabelTag>

          <h2
            className="mb-5 font-display text-[clamp(2rem,3.5vw,2.8rem)] font-semibold -tracking-[0.01em]"
            dangerouslySetInnerHTML={{ __html: title }}
          />

          {content.map((p, i) => (
            <p key={i} className="mb-3.5 text-[0.97rem] leading-[1.85] text-muted">
              {p}
            </p>
          ))}
